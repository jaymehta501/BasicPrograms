...
/**
 * 
 * 1) Declare a Stack of type
 * String/character. 2) Now traverse the
 * expression string expression. a) If
 * the current character is a opening
 * bracket (‘(‘ or ‘{‘ or ‘[‘) then push
 * it to stack. b) If the current
 * character is a closing bracket (‘)’
 * or ‘}’ or ‘]’) then pop from stack
 * and if the popped character is the
 * matching opening bracket then fine
 * else parenthesis are not balanced. 3)
 * After complete traversal, if there is
 * some starting bracket left in stack
 * then "not balanced".
 * 
 * 
 *
